...
object SparkStream2 {
  ...
  /**
   * Only Append output mode is allowed if any df is streaming
   * Stream join with static: static side outer is not supported
   * Stream join with stream: Right/Left outer join supported with only with watermark
   * Inner join are supported with/without watermark
   */
